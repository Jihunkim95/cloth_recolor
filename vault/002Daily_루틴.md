...
- 2026-05-15 (금) PM2: [[results/exp028_loss_search|exp028]] — **loss function search with multi-camera SAM3 mapping validation 학습 중 supervise**. 8 GPU Phase 1 jumpingjacks 8 combo sweep: winner `L_BCE + 2.0·L_mapping (rendered Dice)` 가 baseline 0.8797 → **0.9706 val IoU** (+9pp). Phase 2 8 D-NeRF scene 적용: mean val_iou **0.8430 → 0.8946** (+5.2pp). standup 큰 개선 (+34.8pp), lego 회귀 (-3.8pp).
- 2026-05-15 (금) PM3: lego 회귀 디버깅. 원인 = SAM3 prompt 가 minifigure 잡으려는데 lego 는 *yellow bulldozer*. 4 prompt variant 비교: **`"bulldozer, excavator, tractor, front loader"`** 가 val_iou **0.951 (이전 0.589 의 +36pp)** + 시각적으로 bulldozer만 정확 분리. v2_toy 가 highest val_iou (0.973) 지만 baseplate 까지 over-cover → val_iou metric 의 한계 (SAM3 mask 자체 quality 검증 필요). 교훈: scene 별 prompt audit + human review 병행 필수.
